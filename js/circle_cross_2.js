class Base { // The information of the game will be stored in this class
    constructor() {
    ...
    checkWin(){
        ...
        for (let winListElement of this.winList){
            ...
            if (this.getArea(a) === this.getArea(b) && this.getArea(b) === this.getArea(c) && this.getArea(a) !== 0){
    ...
    reset(){
        ...
        this.round = 0;
        [...document.getElementsByClassName('area')].forEach(
            (area) => {
    ...
    heightWinningChance(emptyArea){
        ...
        for (let emptyAreaElement of emptyArea){
            ...
            for (let winListElement of this.winList){
                let [a, b, c] = winListElement;
                if (a === emptyAreaElement || b === emptyAreaElement || c === emptyAreaElement){
                    if (this.getArea(a) !== 1 && this.getArea(b) !== 1 && this.getArea(c) !== 1){
                        winningChance++;
                    }
                }
            }
    ...
    (area) => {
        area.addEventListener('click', () => {
            if (base.getArea(area.id) !== 0) {
                alert(`This area has been click by ${base.getArea(area.id) === 1 ? 'player_1' : 'player_2'}`);
            ...
            console.log(player);
            if (base.checkWin() === 1){
                alert(`Player ${player === 1 ? 'player_2' : 'player_1'} wins!`);
                return; // If there is a winner, give an alert
            }
            if (base.round === 9){
                alert('Draw!');
